...
>
> **Единственное каноническое место для ПРАВИЛ работы** — этот файл (§14 и далее).
> worklog.md — только хроника (история), НЕ источник правил.
...
**Дата:** 2026-09-15
**Версия:** v0.2.x
**Стадия:** игровые системы живые, подготовка к плейтесту боевки

**Что работает:**
- Core (DI, EventBus, ~130 контрактов) + 16 Modules — бо́льшая часть не-stub
- Body/Qi/Combat/NPC/Inventory/Trade/Quest/Save — игровые циклы
- R17 save/load: 19 ISaveable-блоков, RestoreOrder, атомарный tmp→rename, автосейв
- INP-1 модальная пауза: инвариант «пауза ⇔ стек окон ∨ Esc» (см. 09_15_inp1_modal_freeze.md)
- **Основной мир: large_world 500×500** (полигон 50×50 — второй кнопкой/QA)
- QA-регрессия: 17 симов PASS через `tools/qa_regression.sh` (~2 мин)
- Аудиты 09-11 (14 дефектов) и 09-15 (3 P1 + 4 P2) — исправлены, закоммичены

**Что в работе:**
- Плейтест боевки пользователем в большом мире 500×500
- R18-очередь: F-5, CH-4, SAV-4, SAV-6, G-1+NPC-6, NPC-2, трупы на земле вне сейва

**Что НЕ начато (дальние планы):**
- Процедурная генерация уровней/подземелий
- Контент (lore, items, техники — широкий масштаб)

**Подробный статус:** `SESSION_SUMMARY.md`, `worklog.md` (хвост), последний чекпоинт.
...
> **Проблема:** песочница Z.ai Code периодически обрывает сессию
> (`server_shutdown`) без предупреждения. Незакоммиченная работа теряется.

### 14.1. Наблюдения (зафиксированы 2026-09-15, три обрыва за день)

| Обрыв | Время (UTC) | Вероятная причина |
|-------|-------------|-------------------|
| 1 | ~08:10 | **Параллельный** запуск 3 субагентов (~510K токенов суммарно, пиковая нагрузка) |
| 2 | ~10:00 | Кумулятивный контекст длинной сессии (восстановление + 3 последовательных аудита по 128–169K + правки) |
| 3 | ~10:45 | Продолжение того же длинного дня после SCHEME-коммита ee56cc2 (10:38) — кумулятивный контекст |

- Живые отрезки сессии: **20–80 минут**.
- Счётчика токенов main-сессии нет; субагенты отчитываются сами (≈128–169K каждый).

### 14.2. Правила (обязательны в каждой сессии)

1. **Эпизод ≤ ~45 мин работы → коммит + пуш + чекпоинт.** Обрыв теряет
   максимум пол-эпизода; GitHub всегда актуален.
2. **Субагенты — ТОЛЬКО последовательно.** Максимум 1 агент за раз.
   Параллельный мульти-запуск запрещён (главный подозреваемый обрыва 1).
3. **Компактный контекст:** grep-фильтры вместо полных логов, точечные
   чтения файлов (offset/limit), минимум многострочного вывода в чат.
4. **Метка `NEXT:`** в конце каждого worklog-эпизода — точка возобновления
   после обрыва (что делать первым шагом).
5. **Тяжёлые прогоны** (полная регрессия ~2–3 мин, длинные build) —
   **в конце эпизода**, чтобы обрыв не потерял правки.
6. **Время в чат на каждом промежуточном этапе** (`date` — требование
   пользователя 2026-09-15): старт эпизода, коммит, конец эпизода.
7. **Push на GitHub** после каждого эпизода: токен из
   `/home/z/my-project/.auth/github.token` (см. запрет №6).
...
3. `git log --oneline -5` + `git status` — что уже в git, что в рабочем дереве.
4. Продолжать с `NEXT:`, зафиксировать потерянное (если было).
