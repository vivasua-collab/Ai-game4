...
namespace CultivationGame.Core.Data
{
    ...
    /// <summary>
    /// R25 (2026-09-21, план R23 §2.1): форма площадной техники (Subtype=RangedAoe).
    /// None — техника НЕ площадная (все существующие техники; обратная
    /// совместимость сейва: int 0 → None).
    /// Геометрия — int-математика тайлов (ЗАПРЕТ 3.9): круг/полукруг —
    /// Чебышёв, конус — квадрат скалярного произведения (AoEResolver).
    /// </summary>
    public enum AoeShape
    {
        None = 0,
        Circle,        // Круг с эпицентром в точке прицеливания (курсор/цель)
        Cone,          // Конус от кастера к точке прицеливания (halfAngle 30/45/60)
        Semicircle,    // Полукруг перед кастером (halfAngle 90)
        Line           // Луч/линия от кастера (ширина ~1 тайл)
    }
