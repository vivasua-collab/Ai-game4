...
public partial class HotkeysWindow : Control
{
    ...
    private static (string Group, (string Key, string Desc)[] Entries)[] BuildContent() => new[]
    {
        ...
        {
            ("Пробел", "атака"),
            ("Tab", "выбор цели (цикл: рамка-подсветка; атака бьёт выбранную)"),
            ("G", "стойка защиты: уклонение → парирование → щит → нет"),
            ...
            ("1 / 2", "переключить режим боя: ближний / дальний (лук)"),
        }),
        ("Взаимодействие", new[]
        {
            ...
            ("ПКМ (в инвентаре)", "использовать предмет (камень Ци — поглощение)"),
        }),
        ("Окна", new[]
        {
            ("B", "инвентарь"),
            ...
            ("J / Q", "журнал событий / журнал заданий"),
        }),
        ("Состояние практика", new[]
        {
            ("V", "медитация (поглощение Ци из среды; спец-слот хотбара)"),
        }),
        ("Система", new[]
        {
            ...
            ("F1", "это окно справки"),
            ("F3", "FPS-счётчик (диагностика подлагиваний)"),
            ("F7", "индикация врагов вкл/выкл (HP-бары, урон над врагами)"),
        }),
        ("Камера", new[]
        {
            ("Колесо мыши", "масштаб камеры"),
            ("Средняя кнопка", "сброс масштаба (×3)"),
        }),
        // FIX (аудит 2026-09-06): из перечня убраны нерабочие проводки, чтобы
        // справка не обещала того, чего нет (M/N — карты, R — отдых, F5/F9 —
        // сейвы отключены решением Q8, ` — лог ввода). Окно — источник
        // истины для игрока: рекламировать можно только работающее.
        ("В разработке / отключено", new[]
        {
            ("M / N", "карта мира / мини-карта — не реализовано"),
            ("R", "отдых — не реализовано"),
            ("F5 / F9", "запись / загрузка — отключено (сейвы отложены)"),
        }),
        ("Разработка (DEBUG)", new[]
        {
            ("F2", "чит-меню (отключается в настройках главного меню)"),
        }),
    };
