...
Stage Summary:
- Чекпоинт: checkpoints/09_15_inp1_modal_freeze.md.
...
---
## L500 — мир 500×500 основной + интеграция генераций NPC (2026-09-15)

Task: запрос пользователя: «Основным миром становится мир 500 на 500. В него
необходимо интегрировать все генерации NPC, в нем я буду тестировать боевку».

Work Log:
- Меню: «Новая игра» → large_world (основной), полигон — второй кнопкой;
  GODOT_NEWGAME (QA) → test_polygon; новый хук GODOT_NEWGAME_WORLD оверрайдит.
- GenerateStartup: масштаб от площади (area ≥100k → scale до ×4, кап 12×scale);
  малые карты — scale=1 с идентичным rng-стримом (QA-детерминизм полигона).
- Звери: area/10000 (500×500 → 24); попытки ×4.
- «Пояс жизни»: 60% NPC (±60)/зверей (±80)/групп (±90) у центра карты —
  игрок встречает население, а не марширует по пустыне.
- L500SimDebug (GODOT_L500_DEBUG): сетка/NPC≥30/живые≥15/звери≥15/группы≥4/
  пояс≥40%/кап≤100; раннер сам ставит GODOT_NEWGAME_WORLD для L500.
- QA: build 0 err; L500 PASS (33 заспавнено/26 живых — эмерджентная
  смертность диких земель = фича; звери 33; группы 4; пояс 73%);
  полная регрессия 17/17 PASS — детерминизм полигона НЕ сломан.

Stage Summary:
- Чекпоинт: checkpoints/09_15_l500_primary_world.md; docs_v2 sync
  (TESTING_RULES §0.1 36 хуков + §0.2; MODULE_STRUCTURE NPC-блок L500).

---
## SCHEME — короткие сессии против зависаний песочницы (2026-09-15)

Task: запрос пользователя: «Необходимо разработать схему коротких сессий,
чтобы исключать зависания работы» + вести учёт времени/токенов.

Наблюдения (09-15, три обрыва):
- Обрыв 1 (~08:10): ПАРАЛЛЕЛЬНЫЙ запуск 3 субагентов (~510K токенов
  суммарно) — пиковая нагрузка.
- Обрыв 2 (~10:00): длинный заход (восстановление + 3 последовательных
  аудита по 128-169K + правки) — кумулятивный контекст сессии.
- Продолжительность живых отрезков: 20-80 мин; токены субагентов видны
  в их отчётах (169K/128K/147K), счётчика main-сессии нет.

СХЕМА (правила работы ассистента в этой песочнице):
1. Эпизод ≤ ~45 мин работы → коммит+пуш+чекпоинт. Обрыв теряет максимум
   пол-эпизода; git всегда актуален (сегодня: 5e56d4a → b4f0e91 → 06adf81).
2. Субагенты — ТОЛЬКО последовательно (параллельный мульти-запуск =
   главный подозреваемый обрыва 1). Максимум 1 агент за раз.
...
4. Метка «NEXT:» в конце каждого worklog-эпизода — точка возобновления
   после обрыва (что делать первым шагом).
5. Тяжёлые прогоны (полная регрессия ~2:20) — в конце эпизода, чтобы
   обрыв во время прогона не потерял правки (они уже в git).
6. Время в чат на каждом этапе (просьба пользователя 09-15).

NEXT: R18-очередь (F-5, CH-4, SAV-4, SAV-6, G-1+NPC-6, NPC-2, трупы на
земле вне сейва) либо тестирование боевки пользователем в большом мире.

---
## START_PROMPT-канон правил + аудит «код vs сцены» (2026-09-15, эпизод 2)

Task: запрос пользователя: правила — только в START_PROMPT.md (обновить
инфо о проблемах сессий); большой аудит: сложность перехода от кодовой
композиции объектов к сценам (ТОЛЬКО анализ); чекпоинт 09_16_inp1_modal_freeze
переименовать (имя некорректно).

Work Log:
- START_PROMPT.md: §14 «Правила сессий песочницы» — канон (эпизоды ≤45мин,
  субагенты строго последовательно, компактный контекст, NEXT-метки, тяжёлые
  прогоны в конце, время в чат, пуш после эпизода); §14.1 таблица 3 обрывов
  09-15; §14.3 протокол восстановления; §10 статус актуализирован (R17, L500,
  QA 17/17); шапка: правила только здесь, worklog — хроника.
- Чекпоинты → формат ММ_ДД: 09_16_inp1_modal_freeze → 09_15 (создан 15.09,
  дата внутри исправлена); легаси 2026-08-2x → 08_2x (8 файлов + plans/),
  перекрёстные ссылки обновлены (в т.ч. 09_11_audit_generator.md).
- Аудит «код vs сцены» (analysis-only): инвентаризация — 2 пустые .tscn,
  0 PackedScene, ~241 new-нод, 462 AddChild, UI 10.2K + Scene 12.3K LOC;
  docs_v2 UI_DESIGN §7 (решение «UI из кода» + устаревшее «бинарные сцены»);
  3 яруса сложности миграции (~4–7 недель полная); для procgen критичен
  только Ярус 3 (PackedScene-шаблоны сущностей — пререквизит генерации
  уровней); риски (layout-перенос, тема, AI-цикл, QA-пути).
- Коммит 82e762a (эпизод 1: правила+переименования) запушен.

Stage Summary:
- Чекпоинт: checkpoints/09_15_scene_migration_analysis.md (полный аудит).
- Правила сессий канонизированы в START_PROMPT §14 — единственное место.

NEXT: (на выбор пользователя) обсудить выводы аудита сцен / R18-очередь
(F-5, CH-4, SAV-4, SAV-6, G-1+NPC-6, NPC-2, трупы вне сейва) / плейтест
боевки в L500.

---
## START_PROMPT_TEMPLATE — универсальная заготовка для новых проектов (2026-09-15, эпизод 3)

Task: запрос пользователя: сделать из START_PROMPT.md отвязанный обобщённый
файл для старта нового окружения на любую тему; гармонично интегрировать
тезисы базового промпта о правде/достоверности.

Work Log:
- Создан START_PROMPT_TEMPLATE.md (корень, ~390 строк): §0 «Принципы
  достоверности» — все 8 тезисов базового промпта, адаптированы к dev-
  контексту (источник = file:line/commit/лог; статусы [факт]/[гипотеза]/
  [не подтверждено]); §0 НАД всеми правилами + эхо в §6.5/§8/§10/§14.
- Перенесены наработки: быстрый старт с токен-бюджетом, чекпоинты ММ_ДД
  (формат+структура+когда), SESSION_SUMMARY, worklog append-only + NEXT,
  запреты с причинами, статус с подтверждениями, правила сессий §14
  (эпизоды ≤45мин, субагенты последовательно, время в чат), протокол
  восстановления, §15 чек-лист заполнения.
- Все плейсхолдеры {{...}}/TODO; провенанс указан; предметная область
  отвязана (никаких Godot/C#-специфик).

Stage Summary:
- Артефакт: START_PROMPT_TEMPLATE.md — забирается в новый проект как есть,
  дорабатывается агентом по §15.

NEXT: unchanged (R18-очередь / плейтест L500 / обсуждение аудита сцен).

---
## Повторный репорт INP-1: MODAL2-перебор + индикатор паузы (2026-09-19, эпизод 1)

Task: восстановление окружения (сброс 19.09: dotnet отсутствовал, git -4
коммита) + баг-репорт: «снова сломалось перемещение после закрытия инвентаря,
блокиратор не снимает запрет».

Work Log:
- Окружение: dotnet 8.0.425 установлен; git fetch→reset на 53120ea (локальные
  R17-правки уже в 5e56d4a, патч-страховка /tmp/lost_work_r17.patch).
- Код-ревью всех путей паузы/резюма GWC — дыр не найдено.
- MODAL2 (GODOT_MODAL2_DEBUG): 60 пар стеков + Esc-инвариант + тройной стек —
  PASS (логика модальной паузы полна, headless НЕ воспроизводится).
- Индикатор паузы с причиной: «⏸ ПАУЗА — B·инвентарь» / «⏸ ПАУЗА (Esc)» +
  печать перехода в лог. Пауза была невидима — недиагностируемый репорт.
- Регрессия 18/18 PASS (~2.5 мин).

Stage Summary:
- Чекпоинт: checkpoints/09_19_inp1_recheck_modal2.md.
- Версии причины репорта: Esc-пауза по инварианту (теперь видима) |
  runtime-специфика | старая сборка у пользователя. Ждём тест с индикатором.

NEXT: локальный тест пользователя на свежей сборке с индикатором паузы;
при повторении без индикатора — полный лог (баг вне паузы). Затем (из
прошлых NEXT): тестовая задача переноса UI в сцену (лёгкая схема) +
Godot 4.7.2 changelog/оценка — обе задачи из 16.09 ещё не начаты.

---
## Godot 4.7.2: восстановление потерянного исследования + оценка обновления (2026-09-19, эпизод 2)

Task: «у нас был этап исследования, Godot 4.7.2 есть ли смысл обновляться,
не могу найти документы по результатам — посмотри, если нет, оцени заново».

Work Log:
- Поиск исходных документов по всему репо/worklog/tool-results/git
  (grep 4.7.2|4.7.1, reflog, stash): НЕ найдены — единственный след
  NEXT-метка «задача ещё не начата». Исследование 16.09 потеряно с
  обрывом контекста, документов создано не было.
- Повторное исследование по живым данным: статья релиза godotengine.org
  (18.08.2026) + RC1-статья + веб-поиск; GitHub API — rate-limited.
- Факты: 4.7.2 stable 18.08.2026, 57 фиксов/39 контрибьюторов,
  официальное «no known incompatibilities with 4.7.1, we encourage all
  users to upgrade», .NET-сборка остаётся на .NET 8, 4.8-dev не для нас.
- Релевантность: 2 input-фикса касаются Windows-локальных-тестов
  пользователя (high-polling мышь — critical; simultaneous shift
  release — у нас Shift задействован), core-стабильность; C#/.NET,
  TileMap, 2D, UI — НЕ затронуты (ни одного точечного фикса под наш
  стек).
- Цена: cold_start.sh (URL/size-check/пути) + run_godot/qa_regression +
  5 доков + подмена бинарника + QA 18/18 — один короткий эпизод.
- Код НЕ менялся (research-only).

Stage Summary:
- Чекпоинт: checkpoints/09_19_godot_472_upgrade_eval.md (в этот раз
  исследование СОХРАНЕНО в репо — Lesson Learned 16.09).
- Вердикт: ОБНОВЛЯТЬСЯ ДА, без срочности (safe maintenance-прыжок,
  Windows-ввод-польза для локальных тестов; для нашего стека — ни одного
  стек-специфичного фикса). План исполнения — §6 чекпоинта.
- Важная увязка: решающий тест INP-1 у пользователя лучше делать на той
  же версии движка, что и песочница (§4.3 чекпоинта).

NEXT: решение по обновлению — за пользователем (обновление = ~30-мин
эпизод по плану §6). Параллельно: тестовая задача переноса UI в сцену
(из 16.09); ждём локальный тест INP-1 со сборкой-с-индикатором.

---
## Godot 4.7.1 → 4.7.2: обновление окружения + INP-1 статус (2026-09-19, эпизод 3)

Task: «Проводим обновление на новую версию (посмотри все актуальные
релизы). Если доступна — обнови все стартовые скрипты и восстановление
окружения сразу на версию 4.7.2. Продолжаем работу над игрой».

Work Log:
- Проверка доступности: HTTP 200, zip 107698034 байта, бинарник
  146900080 байт, 4.7.2.stable.mono.official.ed1daf0bf (запрошено
  живым curl до релиз-asset GitHub).
- Скачан и извлечён python-zipfile (dot-нормализация), старый каталог
  4.7.1 удалён, симлинк /home/z/godot указывает на реальную
  my-project/godot (там теперь только 4.7.2).
- Обновлены: cold_start.sh (URL/GODOT_BIN/size-check 146900080/
  zip-имя/комментарии), tools/run_godot.sh, tools/qa_regression.sh,
  game/CultivationGame.csproj (Godot.NET.Sdk/4.7.1→4.7.2),
  2.5d-demo csproj, START_PROMPT.md (8 мест), README.md, docs/README.md,
  ENVIRONMENT_LINKING.md, COLD_START.md, SESSION_SUMMARY.md (строка
  «Замороженные решения» + шапка), 2 godot-reference индекса.
  Исторические чекпоинты/docs_temp/worklog — НЕ тронуты (хроника).
- NuGet: Godot.NET.Sdk 4.7.2 + godotsharp 4.7.2 скачаны в кэш.
- dotnet build: 0 errors (415 warnings — прежние, известные).
- Headless boot 4.7.2: InputMap/World/TileService — чисто.
- **QA-регрессия 18/18 PASS** (COMBAT_SIM, COMBATAI, ANIMALQA, LOOT,
  DOT, SAVELOAD, KILLFEED, QUEST, STORAGE, HOTBAR, TRASHDROP, CONTEXT,
  MODALQA, MODAL2, L500, WEAPONVIS, REASSEMBLY, CHARGE).

Stage Summary:
- Движок проекта теперь 4.7.2 (.NET 8) — песочница и QA-харнесс
  переведены; пользователю для локальных тестов: скачать 4.7.2 .NET,
  кэш .godot перегенерится сам, сейвы не затронуты.
- **INP-1 СТАТУС (репорт пользователя)**: «ошибка с зависанием очень
  похожа на паузу, которая не отжималась. Сейчас при активном
  отображении включения паузы — не воспроизводится, так как я вижу
  состояние игры». → Подтверждение гипотезы-лидера (невидимая пауза);
  HUD-индикатор сделал её видимой и диагноструемой. INP-1 закрываем
  как «подтверждено-починено», возвращаемся при рецидиве с полным
  логом.
- Следующие задачи (запрос пользователя): (1) полоски HP врагов над
  головами, глобально отключаемые (на высокой сложности — никакой
  индикации урона/HP противников); (2) анализ системы ходов в бою —
  уход от «ходов» к скорости атаки оружия (1 укус волка против 2
  ударов игрока = разные скорости атаки), большая аккуратная правка
  физической боевки.

NEXT: HP-бары врагов (глобальный тумблер) + анализ боевой системы
ходов → редизайн на attack speed (субагент-анализ, потом правки).

---
## R18-1: индикация врагов (HP-бары + глобальный тумблер) + анализ attack-speed (2026-09-19, эпизод 4)

Task: «1. Полоска жизни над врагами, глобально отключаемая (высокая
сложность — без индикации урона и жизни противников). 2. Анализ системы
ходов в бою — уход к скорости атаки оружия (волк кусает раз — я стукаю
два раза), скорость атаки закладывалась при проектировании оружия».

Work Log:
- Открытие: HP-бар над NPC уже был (Phase 7 + nameplate S1), но только
  при hp<max, без тумблера; у животных не было; цифры урона без гейта.
- GameSettings: + ShowEnemyVitals (persists, user://settings.json).
- NPCSpriteRenderer: гейт + показ при hp<max ИЛИ NPCState.IsInCombat
  (полный HP в бою = враг); QA-API WouldDrawNpcHealthBar.
- AnimalSpriteRenderer: + HP-бар (стиль NPC), условие hp<max ИЛИ
  IsHostile; inject IBodyDataProvider; QA-API WouldDrawAnimalHealthBar.
- DamageNumberRenderer: гейт ВСЕХ текстов над НЕ-игроком (урон ПО игроку
  остаётся); QA-счётчик EnemyTextSuppressedCount.
- F7: InputMap → sticky → IsEnemyVitalsTogglePressed → GWC-хендлер
  (тост+лог+save); HotkeysWindow строка; CheckButton в настройках меню.
- ANIMALQA шаг 8: первые 2 итерации флаковали через ход-гейт
  CombatService — перепроектирован детерминированно (прямая мутация
  non-vital частей + синтетический DamageAppliedEvent с фейковой целью).
- Прогоны: ANIMALQA 3× PASS, QA-регрессия 18/18 PASS (на 4.7.2).
- Параллельно субагент COMBAT-AS-ANALYSIS: анализ боевой системы ходов +
  проект attack-speed (readiness-модель промилле, план 3+1 эпизода,
  12 инвариантов) — чекпоинт 09_19_combat_attack_speed_analysis.md.
- docs_v2: HOTKEYS §2.1 (+F7), TESTING_RULES §0.1 (ANIMALQA шаг 8).

Stage Summary:
- Чекпоинт: checkpoints/09_19_r18_enemy_vitals.md.
- Фича готова к локальному тесту пользователя (на 4.7.2): F7-тумблер,
  бары над волками с первой стычки/урона, OFF — «как на высокой
  сложности» (ни баров, ни цифр над врагами; урон по игроку виден).
- AttackSpeedFactor из проектирования оружия НАЙДЕН наполовину
  (в таблицах есть, потребляется как урон-компенсация; в предметы не
  копируется, геттера нет) — гипотеза пользователя подтверждена.
- INP-1 закрыт подтверждением (см. эпизод 3).

NEXT: ревью пользователем readiness-модели (порог 1000‰, волк 500‰,
кулаки 1000‰, техники без расхода готовности) → эпизоды миграции
attack-speed по плану (данные+ядро → адаптеры → QA → доки). Локальный
тест R18-1 + 4.7.2 у пользователя.
---
## R19 «Потребляемые ресурсы»: ПКМ-«Использовать» с маршрутизацией + читаемость подокна (2026-09-19, эпизод 5)

Task: «Я поднял лекарство, у меня нет пояса, должна быть возможность
потребить его по правой кнопке. 1. Более выраженный фон подокна — не
читается текст. 2. Кнопка "Использовать" — эффект зависит от характеристик
и типа объекта (лекарства/растения/свитки/еда). Проведи аудит — возможно,
есть реализованные, но не внедрённые функции (для пыли хаотичного Ци
использование уже есть)».

Work Log:
- Аудит: гипотеза пользователя ПОДТВЕРЖДЕНА — BeltService.ApplyEffect
  (heal/qi_restore) реализован, но работает только через пояс; TryUseQiStone
  — только камни Ци; «Использовать» в ПКМ-меню — только у камней Ци.
- Пиксель-анализ скриншотов пользователя: в области панели НЕТ пергамента
  #e8d5a8 (фон #3c3120, текст #7f7f7f, контраст ~2.9:1) → КОРНЕВОЙ БАГ:
  Panel с CustomMinimumSize(400,0) без контейнера = высота 0 → StyleBox-фон
  не рисовался вовсе, контент «плавал» на бекдропе. Кнопки были видны
  (Button имеет min-height от текста) — «кнопки есть, фона нет».
- Ещё 2 скрытых бага аудита: (а) ItemGeneratorService пишет EffectType="Heal"
  (заглавная) — сгенерированные лекарства НЕ ЛЕЧИЛИ (switch по "heal"), меню
  показывало сырой ключ («Неал +15» на скрине = «Heal +15»); (б)
  StartingGearPhase: Effects стартовых расходников НЕ заполнялись (effect из
  тапла игнорировался, value шёл в стоимость) — пилюли были «пустыми».
- Новое ядро: IItemUseService + ItemUseService (Modules/Inventory) — единая
  маршрутизация по типу: камни Ци → поглощение (риск хаоса перенесён),
  heal → по повреждённым частям, qi_restore → AddQi, material/неизвестные →
  не используется. Кейс-нормализация ключей в ОДНОМ месте. DI зарегистрирован.
- ItemContextMenu: PanelContainer (авторазмер) + явный stylebox-override
  (пергамент + рамка золотая 3px) + бекдроп 0.45 + InkBlack 13px для
  описания; кнопка «⚡ Использовать» для всех используемых типов (первичная,
  во всю ширину, tooltip с описанием действия); эффекты — нормализованные.
- Интеграция: InventoryWindow.TryUseItem(slotId) + GetUseInfo + TryUseQiStone
  (обёртка); BeltService.Use — делегирование IItemUseService (ApplyEffect
  удалён, IQi/IBody инжекты убраны); StartingGearPhase — Effects в данных,
  живые описания, cost отдельным полем.
- QA: CONTEXT-сим 11→15 шагов: поглощение камня сквозь кнопку (с дрейном Ци —
  буфер полон, кап MaxQi), пилюля из ПКМ-меню (урон→heal→счётчик −1),
  лекарство "Heal" ЗАГЛАВНОЙ (репродукция бага пользователя — нормализация
  работает), материал без кнопки, читаемость (высота>100/альфа≥0.95/
  контраст≥4.5). + GODOT_CONTEXT_MENU_ITEM для скриншотов.
- Прогоны: build 0 errors; CONTEXT 15/15; QA-регрессия 18/18 PASS (4.7.2).
- Визуальная верификация: Xvfb-скриншоты + VLM — панель с рамкой читаема,
  кнопка «Использовать» на месте, «Лечение +30» (нормализовано), у материала
  кнопки нет; пиксель-анализ: пергамент #e8d5a8 теперь присутствует.

Stage Summary:
- Чекпоинт: checkpoints/09_19_r19_consumables_use.md.
- Потребление расходников БЕЗ пояса работает (ПКМ → «Использовать»);
  маршрутизация расширяема под растения/еду/свитки (ключи будущих фаз).
- Три скрытых бага закрыты: не-рисовавшийся фон ПКМ-панели, кейс "Heal"/"heal"
  (генератор vs потребители), пустые Effects стартовых расходников.
- Доки: INVENTORY_SYSTEM §7.2/§9/§9.2, TESTING_RULES (CONTEXT 15/15 + env).

NEXT: локальный тест пользователя (ПКМ-лекарство: фон + Использовать);
в очереди — перенос UI в сцену (16.09) и attack-speed миграция по плану
чекпоинта 09_19_combat_attack_speed_analysis.md.
---
## R20 «Восемь багов плейтеста» — эпизод 1 из 2 (2026-09-19/20, ~23:00–00:35 МСК)

Task: запрос 09_09_22_40 — 8 багов локального плейтеста + мета-указания
(чекпоинт-план ДО работы, отметки по ходу; без субагентов).

Work Log:
- Протокол: дата/время/токен/START_PROMPT.md/SESSION_SUMMARY прочитаны
  (19:57 UTC). Чекпоинт-план создан ДО правок: checkpoints/09_19_r20_eight_bugs_plan.md.
- №1 «Наталья»: генератор имён УЖЕ работал (QA: 12 различных на 25) — у
  пользователя устаревшая сборка. Улучшено: пул 8+8 → 24+24 + 20 фамилий,
  дедуп сессии (коллизия → «Мария Ветрова»), ResetSession при пересборке.
  QA: 25/25 уникальных, топ 4%.
- №3 «волки умирают от старости»: 3 корня — DetermineAge клампил возраст
  существ к ≥16 (человеческий минимум) при lifespan волка 10-15;
  WorldModule._lastYear=-1 → фантомный YearChanged на первом тике (мгновенное
  старение); OnYearChanged убивал по age ≥ lifespan. Фиксы: возраст вида без
  клампа (16+ только люди), init календарных маркеров в Start, old_age-смерть
  УДАЛЕНА (правило пользователя: NPC умирают только в бою/от проклятий/ядов).
  QA: 0 old_age.
- №2 «NPC статичны/не преследуют»: телеметрия L500 — блуждание = осцилляция
  у якоря (движение работает). Реальные дефекты: (а) NPC в эмерджентной стычке
  игнорирует удар игрока (RetaliateOrFlee early-return) → ретаргет по угрозе;
  (б) погоня 2.4 т/с < игрока 2.8 → ChaseSpeedMultiplier 1.6 (4.8 т/с) +
  DefaultMoveSpeed 2→3; (в) leash 15т → 30т (AggroRadius×6). QA COMBATAI+1b:
  погоня 6→1 тайл за 4с.
- №4 «Мёртв при полуживом»: IsVital-флаг ТОРСА в «правиле смерти» — торс
  0/100 = «Мёртв» (скрин: Head 50/50, Heart 80/80) + отключение регенерации.
  Канон = Head|Heart (Die-путь, BOD-2). Фикс: IsFatalPart(Head|Heart) в
  BodyDamageCalculator.IsAlive + BodyService.IsEntityAlive.
- №6 «K не закрывается по Esc»: GWC Esc-цепочка не содержала K-окно.
  Добавлена ветка + QA MODALQA №9 (открытие без паузы + Esc-закрытие).
- №7 «медитация 1 тик»: QiModule авто-завершал при IsFull. УДАЛЕНО (не из
  спеки §5.2): медитация продолжается, поглощение капится, тост при активации
  полным ядром. Прерывания: V/бой/ResetWorld.
- №8 «цифры есть, HP не меняется»: пайплайн ЦЕЛЕН в песочнице (CHARGE 11b:
  HP тела 500→450) — регресс-гард добавлен (прежде тест видел только событие
  урона, не тело). Гипотеза для живого теста: 1v1-лок CombatService в большом
  мире (эмерджентные стычки отклоняют касты игрока ход-гейтом) — NEXT.
- №5 (книга техник UI) — не начат, в NEXT эпизода 2.
- Регрессия: 18/18 (ANIMALQA-флейк повторным прогоном подтверждён PASS;
  L500 шаг 7 движения — диагностика без гейта: метрика чистого смещения
  непригодна для осцилляции у якоря; по-настоящему статичные Wandering-NPC —
  расследование NEXT).

Stage Summary:
- Закрыты: №1 (улучшение+QA), №2 (фикс+QA), №3 (фикс+QA), №4, №6 (фикс+QA),
  №7 (фикс). Частично: №8 (регресс-гард+анализ). Не начат: №5.
- Коммит будет запушен (см. ниже). Пользователю: локальная сборка
  устарела (скрины = pre-R19) — pull + rebuild перед проверкой багов.
...
---
Task ID: R21-EP2-0920
Agent: main (Z.ai Code)
Task: Репорт 20.09 №2: миграция боевой системы с ходов на модель скорости атаки (readiness) + clash-парирование «при одновременной атаке».

Work Log:
- Ходовая модель удалена: гейт владения ходом/передачи хода/EnemyTurnTimeout/кулдауны адаптеров (NPC 1.6с, волк 2 тика, игрок 1с) → readiness-словарь (начисление в UpdateTimer, расход только при приёме, кап=порогу).
- attackSpeedPermil = WeaponMain.AttackSpeedPermil (новое поле, генератор: AttackSpeedFactor×1000, кинжал 1300/меч 1000/двуруч 750) × AGI (1000+AGI×10)/1000; кулаки 1000; звери 500‰ (видовой). Инициатива: инициатор с полной готовностью.
- CLASH: защитник в замахе (per-attacker pending-каст) → гарантированное парирование (×0.5), замах не отменяется. Обычные удары не блокируются.
- Per-attacker pending-касты (прежде combat-wide «Каст уже идёт» блокировал параллельные удары); LOS-гейт — IsEntityCasting(атакующего).
- Инциденты: цикл DI (AnimalService+ICombatService — откат на тиковый троттлинг); расход готовности до гейта каста; кап 3000 давал залп.
- COMBAT_SIM: WaitForReadinessAsync + фаза 3e readiness-гейт (charged-путь для детерминизма). QA 18/18 PASS.
- Доки: COMBAT_SYSTEM §1.4/§1.4.2/§8.2, ALGORITHMS, ANIMALS §5.2-5.3, TESTING_RULES; чекпоинт 09_20_r21_attack_speed.md.
...
---
Task ID: R23-MAIN-0920
Agent: main (Z.ai Code)
Task: «Продолжаем боевую систему: фиксы по аудиту при необходимости +
система мультибоя и площадные техники (конус/полукруг/круг), в планах
самонаводящиеся (выбор цели). Анализ, планирование, несколько идей —
пользователь выбирает».

Work Log:
- 10:15 UTC старт. Аудит R22 прочитан; выбран фикс CMB-1 (независим от
  дизайна мультибоя); CMB-2/CMB-3 отложены до выбора варианта мультибоя
  (их фикс зависит от решения).
- R23-1 CMB-1 (коммит 2a50578): charged-атаки (isCharged ∨ potency>базы)
  пропускают readiness-гейт и НЕ расходуют готовность (§8.2 «зарядка была
  замахом»); рефанд: TechniqueService.RefundUse (кулдаун+мастерство;
  Ци рассеялась — не возвращается) + PlayerTechniqueCaster.OnAttackRejected
  (окно 2с, techniqueId-матчинг).
- ИНЦИДЕНТ (среда): песочница сбросила .NET SDK и «загасила»
  my-project/godot (dentry-ghost: записи недоступны при живом листинге,
  exec ENOENT; реальный layout = linux.x86_64 с ТОЧКОЙ после нормализации
  cold_start — «подчёркнутый» каталог в ls = призрак). Восстановление:
  cold_start.sh + стабильная копия в /home/z/godot_bin (вне зоны
  снапшота; zip пере-скачан, python-zipfile извлечение).
- R23-1b QA-детерминизация (коммит db194cf): COMBAT_SIM 3e перестроен —
  DebugSetReadinessPermil (QA-паттерн R16: игровой тик начисляет целую
  секунду готовности, окно «не готов» поллами не поймать); (a) basic
  Accepted → (b) 0‰ Rejected → (c) charged при 600‰ Accepted (CMB-1) →
  (d) readiness=600‰ ПОСЛЕ charged (не расходует — гард) → (e) re-ready;
  WaitForOwnCastClearAsync (IsEntityCasting — чужой NPC-каст не блокирует);
  qa_regression.sh: GODOT_BIN-оверрайд + fallback godot_bin.
- Прогоны: build 0 err; COMBAT_SIM PASS (все 5 шагов 3e зелёные);
  полная регрессия 18/18 PASS.
- Дизайн-анализ (эпизод 2): факты архитектуры собраны (readiness/
  pendingCasts уже per-entity; RangedAoe уже в enum без логики; курсор
  уже в событиях). Документ с вариантами:
  checkpoints/plans/2026-09-20_r23_multicombat_aoe_design.md.

Stage Summary:
- CMB-1 закрыт полностью (гейт+расход+рефанд+QA-гарды); CMB-2/CMB-3
  ждут выбора варианта мультибоя.
- МУЛЬТИБОЙ: A «приоритет игрока» (~0.5 эп, 1v1 остаётся) / B «per-pair
  сессии» (~400-600 строк, целевая архитектура) / C «реестр входов»
  (★ рекомендация: гейт участника долой, NPC-бои молча, UI-сессия
  игрока; 1-2 эпизода; миграция C→B чистая).
- AoE: модель данных (AoeShape/Radius/HalfAngle/Falloff/MaxTargets);
  резолв A «мгновенный залп» (★: полный пайплайн per-target, int-
  геометрия, AoeImpactEvent для VFX) + B «живые зоны» (фаза 2, DoT).
- САМОНАВОДЯЩИЕСЯ: TargetingService (Tab-цикл, курсор) + A «снаряды в
  UpdateTimer» (★, кайт работает) / B «отложенный хит» (заглушка).
- Roadmap: R24 мультибой C → R25 AoE A → R26 Targeting → R27 хоуминг →
  R28+ графика боя. 5 вопросов пользователю в §5 документа.

NEXT: ждём выбор пользователя (мультибой A/B/C; AoE friendly-fire;
эпицентр круга; хоуминг A/B; приоритет порядка). Затем эпизод R24.

---
Task ID: R23-EP3-0921
Agent: main (Z.ai Code)
Task: дата/время; восстановление среды (инцидент повторился); интернет-
исследование массовых боёв (DF/RimWorld/M&B/attack-tokens); ответы:
C→B миграция, friendly-fire месть/ауры, индикация заряда + кнопка спуска,
хоуминг A; обновление дизайн-дока R23 + роадмап R24+.

Work Log:
- 06:35 UTC старт. ИНЦИДЕНТ среды повторился: dotnet сброшен,
  /home/z/godot_bin пуст. cold_start.sh восстановил всё (build 0 err,
  headless-бут ОК). Godot в my-project/godot (реальная директория)
  пережил сброс.
- git push НЕ выполнен: токен-плейсхолдер (__workspace...) в
  my-project/.auth, env пуст, /home/sync висит (network mount).
  Коммиты 2a50578..24df21a локально, push отложен до валидного токена.
- Код-аудит UI: HotbarPanel (654 стр) — ТОЛЬКО кулдаун+Qi; события
  Charge Started/Progress/Completed/Cancelled и HeldTechniqueChanged
  подписаны лишь QA-дебаггером ChargeSimDebug → боль пользователя
  подтверждена кодом (ни заряд, ни аура-удержание не видны).
- PlayerTechniqueCaster: спуск = повторное нажатие (aura-hold вариант В,
  08_25); AuraHoldService: 1 слот, декей 1%/тик, рассеивание <QiCost/2.
- NPCAIService.OnDamageApplied (:515-527): месть через DamageApplied
  per-target УЖЕ работает → AoE-friendly-fire по нейтралам = месть
  включается БЕСПЛАТНО при пер-таргет пайплайне.
- Группы NPC есть (GROUP_SYSTEM: HuntingPack/follower/faction-план),
  сопартийцев игрока НЕТ → «щадящие ауры» отложены, хук в R25.
- Веб-ресёрч (web-search + page_reader, 9 поисков): DF (нет боёв-
  сессий вообще, пер-агентные цели), RimWorld (melee пер-пара, 1
  атакующий на клетку на цель = геометрический лимит; melee-block;
  FF safe radius), M&B (толпа без лимитов, позиционирование решает),
  attack tokens (souls-like; для колони-сима не канон), steering
  behaviors (Reynolds seek + turn-rate, anti-«turn on a dime»),
  D&D Careful Spell (лор для SpareAllies).
- Дизайн-док §6 (2026-09-20_r23_multicombat_aoe_design.md): таблица
  практик; C→B миграция = надстройка +1-2 эп (300-500 строк, двойной
  работы нет); friendly-fire месть бесплатна; SpareAllies 10-20 строк
  в резолвере но ждёт party-систему (хук R25); 3 идеи UI индикации
  (слот-центричная ★ / CastBar / аура-медальон) + кнопка спуска
  (Z/кнопка/R); хоуминг A детализирован (turn-budget int); роадмап
  R24 C → R25 AoE → R26 UI заряда → R27 Targeting → R28 хоуминг →
  R29 VFX → R30 B/зоны/ауры.

Stage Summary:
- Среда восстановлена (build 0 err); push заблокирован протухшим
  токеном — нужен токен от пользователя.
- Все вопросы пользователя закрыты в доке §6; рекомендации: C-старт
  (B = надстройка), AoE-A залп + месть нейтралов, UI = идея 1
  (слот-центричная + кнопка «ВЫПУСТИТЬ»), хоуминг A (turn-budget).
- Открытые решения за пользователем: вариант UI-индикации (1/2/3)
  и способ спуска (кнопка/Z/R); порядок R24-R28 можно менять.
...
---
Task ID: R29-MAIN-0921
Agent: main-thread (Z.ai Code)
Task: R29 «Графика боя (VFX)» — план R23 §6.7 (план эпизода:
checkpoints/plans/2026-09-21_r29_vfx_design.md). Сессия началась после
обрыва на «Пишу фазу 3k в CombatSimDebug»: код рендереров уже лежал в
рабочей директории (обрыв до QA/фазы 3k/доков/коммита).

Work Log:
- Верификация состояния: git log (R24-R28 закоммичены и запушены),
  незакоммичены: AoeFxRenderer/HomingProjectileRenderer/ElementPalette
  (новые), SceneBuilder/TechniqueEffectRenderer/PlayerTechniqueCaster
  (правки), план R29. Среда сброшена (dotnet/godot) → cold_start OK
  (Godot был на месте, проверял неверный путь).
- Фаза 3k в CombatSimDebug: (a) AoE-залп → TotalVolleys +1 /
  TotalTargetFlashes ≥ +2, (b) хоуминг-пуск → TotalLaunches +1,
  (c) Hold/Release → TotalHoldAuraChanges ровно +2; vfxOk в VERDICT;
  инжект AuraHoldService; GODOT_VFX_SHOT-скриншоты (Xvfb + opengl3).
- VLM-аудит кадров №1: конус/болт НЕВИДИМЫ (аура видна). Лог вскрыл
  IndexOutOfRangeException в DrawFan (off-by-one: массив steps+1 при
  записи points[steps+1] → нужен steps+2) — исключение рвало _Draw:
  конус И маркеры не рисовались (падали и залпы фазы 3h). Фикс.
- Диагностика болта: TotalDrawCalls-счётчики + трассировка пусков.
  Прогон №2 вскрыл каскад: болт из (1600,1600)=центр карты, цель
  корректна; леаш-сообщения. Гипотеза смерти-респавна опровергнута —
  КОРЕНЬ: GameWorldController._PhysicsProcess каждый кадр
  синхронизирует ЛОГИКУ к _visualPosition → прямой
  PlayerService.SetPosition живёт один кадр («не прилипает», о чём
  прямо говорит комментарий DEBUG_TeleportPlayer). Фазы с ожиданиями
  видели СТАРУЮ позицию: 3h(c) игрок вне конуса, камера вне кадра.
- Харденинг CombatSimDebug: хелпер TeleportPlayer через родителя-GWC
  (DEBUG_TeleportPlayer: логика+визуал+камера); 3h/3k чистые зоны
  КЛАМПЯТСЯ в границы карты (QA-мир 50×50: p+60/p+80 вне — кламп
  разводил логику и визуал по углам, залп «из угла» = 0 целей);
  паузы на прибытие камеры (1.0/0.8/0.6с); 3k: ожидание респавна
  при мёртвом игроке (3.5с) + recenter перед под-фазами.
- Флейки сима (пойманы скриншот-прогонами, headless-маскировались):
  (1) multi(b): цикл ударов делал 0 попыток, если UI-пара уже открыта
  NPC-AI (гонка) → damage=False — фикс: attempt-0 всегда;
  (2) 3h(c) месть соседа: Threats ЗАТУХАЮТ (NPCAIService decay +
  Remove при ~0) — слабый удар (1 урон → threat 2.0) стирается за
  1.2с паузы; событие регистрирует синхронно → замер НЕМЕДЛЕННО
  (3h(b)/(c), печать post-wait как информация);
  (3) 3h(c) урон игроку: HP-дельта врёт (уклонение / попадание в
  разрушенную часть — событие пишет 105, HP 215→215) → критерий
  прироста суммы DamageAppliedEvent + ретрай ×3 + bestAcc (кулдаун).
- Буст читаемости болта (VLM: бледный Air-болт «не снаряд»): ядро
  13px + сердцевина 7px + ореол 16px + хвост крупнее; вспышка
  контакта 4+18px. Кадр хоуминга: 0.45с → 0.25с (llvmpipe-тайминг
  снимал ПОСЛЕ контакта).
- Прогоны: build 0 err; headless COMBAT_SIM VERDICT PASS; opengl3
  скриншот-сессия VERDICT PASS (прогоны 2-9 — итерации фиксов);
  VLM тройка: конус молнии + кольца на 2 NPC ✓, болт с белым ядром
  и хвостом в полёте ✓, огненное кольцо + 4 искры ✓
  (screenshots/r29_vfx_{aoe,homing,hold}.png); ПОЛНАЯ РЕГРЕССИЯ 18/18 PASS.
- Доки: COMBAT_SYSTEM §4.7 (VFX-фаза: события→рендереры, слои,
  инциденты), TESTING_RULES (3k + GODOT_VFX_SHOT в реестр),
  SPRITE_CATALOG §23 (мировые VFX боя), дизайн-док §6.7 R29 ✅,
  план R29 — итоговый баннер.

Stage Summary:
- R29 закрыт полностью: вспышки AoE-форм + маркеры целей, полёты
  хоуминг-болтов (представление, не источник истины), аура удержания,
  ElementPalette, подавление дублей generic-визуалов. NPC-кастеры
  получают боевые вспышки ВПЕРВЫЕ.
- Богатый улов инцидентов сим-харнесса (5 корней, все задокументированы
  в §4.7): off-by-one, SetPosition-откат, границы карты, Threats-затухание,
  HP-кламп. Скриншот-сессии под opengl3 теперь стабильны.
- Записи worklog R25-R28 пропущены (обрыв сессии) — коммиты полные:
  80fbf4c (R25), 7910e1c (R27), 715c087 (R28).
...
---
## 2026-09-22 R32 — внешний аудит 09_22_07_30 (5 фаз): верификация + P1-фиксы

Сессия: 949391c (R31) → R32. Аудит: upload/audit_09_22_07_30.txt (фазы
1/2/3/5 присутствуют, ФАЗА 4 ОТСУТСТВУЕТ в файле — P1-6 восстановлен
анализом; зафиксировано в чекпоинте).

- Верификация всех P1/P2 по коду 949391c + RUNTIME-верификация по прямому
  запросу аудитора (P1-5/P1-8): новый QA-сим Audit0922SimDebug
  (GODOT_AUDIT0922_DEBUG=1, 16 проверок, префикс-прогон = FAIL с
  DEFECT-строками = runtime-подтверждение; постфикс = PASS).
- P1-5 EventBus ПОДТВЕРЖДЁН runtime полностью: исключение подписчика
  покидало Publish, прерывало fan-out, а стэл re-entrant очередь
  исполнялась при ЧУЖОМ Publish и роняла его (сценарий коррупции
  аудита — 1-в-1). Фикс: пер-хендлерная изоляция (лог+продолжение) +
  дрен очереди в finally на внешнем уровне; порядок доставки
  [ping#1, ping#2, other#9].
- P1-8 path traversal ПОДТВЕРЖДЁН runtime для Modules-layer
  SaveFileHandler (файл создан ВНЕ каталога); Adapter-хендлер УЖЕ
  санировал (уточнение для аудитора). Фикс: SanitizeSlotName паритет
  с Adapter (буквы/цифры/-/_).
- P1-6 DI override ПОДТВЕРЖДЁН runtime (stale concrete-ключ
  Modules.SaveFileHandler резолвился в мёртвый fallback). Фикс прошёл
  ДВЕ итерации: наивный prune ломал мульти-форвард-паттерн (Corpse→
  NPCService, Combat→TechniqueService; поймано живыми регрессиями
  COMBAT_SIM/COMBATAI/ANIMALQA + самим симом E2) → финал: pruning
  ТОЛЬКО при RegisterInstance-оверрайде (single-provider), ключи
  Register<> независимы (префикс-семантика). Стражи C3/C4/C5.
- P1-7 DeleteSave: честный bool (три слоя; семантика P2-15 «операция
  выполнена» — попутно закрыт).
- P1-1 TradeService : IWorldResettable (закрытие сессии событием +
  очистка стока; NPC-ID GUID — сценарий «npc_0» аудита невозможен,
  реальный эффект был утечка+висячая сессия).
- P1-4 CorpseService : IWorldResettable (явный контракт; сброс работал
  транзитивно через NPCModule — аудитор не заметил вызов; явный вызов
  из NPCModule.ResetWorld удалён, один владелец).
- P1-2 SaveAndQuit: bool читается; сбой → тост + Playing (не
  Quitting-saved). Вызывающих нет (контракт API).
- P1-3 NPC zero-GC: CopyStatesTo(buffer) + персистентные буферы
  (NPCAI Tick+Defense, NPCMovement, NPCModule attacks); GetAllStates —
  только холодные пути.
- P2 → БЭКЛОГ (10 пунктов) в чекпоинте §4 (указание аудитора: не
  смешивать с P1). P2-9 (zero-GC формулировка) закрыт скорректированным
  комментарием EventBus.
- Чекпоинт для аудитора: checkpoints/09_22_r32_external_audit.md
  (ответы на вопросы P1-5/P1-8, логи префикс/постфикс, вердикты по всем
  пунктам, P2-бэклог, финальная семантика DI-prune).
- QA: сим №19 AUDIT0922 в qa_regression.sh + TESTING_RULES §0.1 (37
  хуков); регрессия 19/19 PASS; build 0 err.

Stage Summary:
- Все 8 P1 аудита закрыты (7 подтверждены+закрыты, P1-4 частично
  опровергнут функционально/закрыт контрактно).
- Инфраструктура hardened: EventBus exception-изоляция, честный
  save/delete, санация слотов, DI override prune, Trade/Corpse
  reset-контракты, NPC zero-GC.
- Поймана и обучена тонкая семантика DI-контейнера: forwarding-ключи —
  lifeline мульти-интерфейсного паттерна (задокументировано в
  Container.cs + страж C5).

NEXT: аудитор → следующая фаза (матрица state→reset→save→restore по
всем 17 модулям — его анонс); P2-бэклог по приоритету пользователя;
...
## R37-c — Зарядник Ци: окно H + мост камней (баг-репорт 23.09, 2026-09-23)

Репорт владельца (после сбоя окружения на «xauth отсутствует — запускаю Xvfb
вручную»): «не могу добавить камни Ци в него, лекарство добавляется без
проблем».

- Диагноз: окна H не было (тост A6 «вставьте в зарядник (H)» обещал
  несуществующее), моста предметы↔домен не было (Charger-домен заряжался
  только из сейва), зарядник маскировался под пояс (IsBeltEquipped = любой
  предмет Belt → BeltSlotRow принимал лекарства и молча отвергал камни),
  домен активировался без зарядника.
- Фиксы: ChargerItemBridge (конверсия QiStoneData↔QiStone по канону
  1024×см³, слот-адресно R10, тосты-причины, режим домена = экипировка);
  ChargerWindow (H, модальное: слоты drag&drop/ПКМ, буфер, тепло, режим);
  input-цепочка charger_window (H); ItemContextMenu «💎 Вставить в
  зарядник»; BeltService.IsBeltEquipped = Armor (armor_belt); RestoreState
  maxQi из сейва; ChargerModule — гейт моста вместо безусловного Activate.
- Контракт анти-дюпа: полный камень извлекается, частичный — отказ
  (тост-причина), пустой рассыпается (стаки ItemId+Count без per-instance).
- QA: сим №20 GODOT_CHARGERQA_DEBUG (ChargerSimDebug) — prefix FAIL
  (checkpoints/logs/09_23_r37c_prefix_charger.log: окно/мост/гейт
  отсутствуют) → postfix PASS (камень 1024 вставляется, домен качает:
  ядро практика 1005→1049, анти-дюп G1-G4, сейв round-trip 1024→1024,
  пояс-контроль armor_belt). Регрессия 19/19 + CHARGERQA (20/20 в
  реестре). Build 0 err. VLM-скрин окна H (Xvfb :99 вручную — xvfb-run
  требует отсутствующий xauth): 3 слота/буфер/тепло/режим читаемы.
- Доки: CHARGER_SYSTEM §12 (статус+допущения), HOTKEYS §2.3 (H),
  INVENTORY_SYSTEM §7.2 (пояс≠зарядник), TESTING_RULES §0.1 (сим №20).
- Чекпоинт: checkpoints/09_23_r37c_charger_qi_stones.md.

NEXT: R37-хвост (сползание чит-панели + сапоги — QA-сим уже закоммичен
отдельным prefix-коммитом, внести фиксы) → финальный аудит 09.22 22:00
(P1-13-hardening, NEW-TIME-1/2) → P2-пачки → интеграционная фаза.

## R37 (prefix-инфраструктура) — чит-панель + сапоги (баг-репорт 23.09 №1-№2)

QA-сим CheatEquipSimDebug (GODOT_CHEATEQUIP_DEBUG=1, упавшая сессия не
успела его закоммитить) — закоммичен как ПРЕФИКС-стенд баг-репорта 23.09
«сползают надписи чит-меню» + «не смог одеть сапоги, броня одевается».

Prefix-вердикт (checkpoints/logs/09_23_r37_prefix_cheatequip.log): FAIL —
- A1/A2/B1/B2: сапоги/нагрудник экипируются (back-end + double-click) — путь цел;
- A3: гейт L5-сапогов при L1-игроке честный (BlockedEvent с причиной);
- C1/C2: геометрия чит-панели в headless СТАБИЛЬНА (сползание — только в
  живом рендере/после многих нажатий, потребуется Xvfb-скрины);
- U2 (ЕДИНСТВЕННЫЙ DEFECT): EquipmentBlockedEvent публикуется с причиной,
  но игрок её НЕ ВИДИТ (0 тостов ⛔) — корень репорта «не смог одеть сапоги»:
  сапоги уровня выше игрока тихо не надеваются, причина скрыта.

NEXT (R37-фиксы): тост причины EquipmentBlockedEvent в GWC (U2),
Xvfb-скрины чит-панели для сползания (GODOT_CHEAT_SHOT), полный вердикт.
