...
### Индикация в UI (R26, 2026-09-21 — идея 1 «слот-центричная», выбор пользователя)

HotbarPanel подписан на события зарядки/удержания (прежде их слушал
только QA-дебаггер ChargeSimDebug — игрок не видел НИ зарядки, НИ
удержания):

- **Заряд идёт:** слот заряжающейся техники — изумрудная полоса
  снизу-вверх (`ChargedQi/QiCost`), подпись «87%» по центру, пульс
  изумрудной рамки. Overcharge (`ChargedQi > QiCost`): янтарная
  полоска сверху слота + подпись «×N.N» (potency/1000).
- **Удержание в ауре:** слот удержанной техники — рамка цвета стихии
  (`HeldTechniqueChangedEvent.Element`, пульс), метка «◉» в углу.
  Спуск = повторное нажатие той же клавиши (Z / 3-9 / клик слота) —
  механика §5.4 без изменений (решение пользователя: без отдельной
  кнопки).
- События: `TechniqueChargeStarted/Progress/Completed/CancelledEvent`
  + `HeldTechniqueChangedEvent` (все — readonly struct, фильтр
  `EntityId == player`, вне боя работают).
