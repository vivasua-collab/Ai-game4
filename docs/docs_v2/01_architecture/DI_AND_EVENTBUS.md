...
### 1.8. Контракты жизненного цикла и ResolveAll (аудит 09.22, Фазы 4/6/7/10 — P2-12/P2-13/P2-14, P1-9/P1-10)

Контейнер, `GameEntryPoint` и фазы сборки мира держат пять явных контрактов
(раньше — молчаливые инварианты, часть не выполнялась):

1. **ResolveAll — порядок регистрации.** Итерация идёт по ordered-списку
   вызовов `Register*`/`RegisterInstance` (не по словарю ключей — после
   удалений prune-ом P1-6 порядок `Dictionary.Values` не гарантирован).
   Мёртвые регистрации (не владеющие ни одним ключом: хвосты prune-а,
   self-bound ключ, украденный форвардом) из ordered-списка удаляются —
   воскреснуть через ResolveAll они не могут. Матчинг — по фактическому
   типу инстанса, дедуп — по ссылке. Следствие: `IStartable`/`ITickable`
   стартуют в порядке регистрации модулей (§1.2), `ISaveable`-агрегатор —
   в стабильном порядке. Страж — QA-сим AUDIT0922 секция F.
2. **Startup — FAIL-CLOSED.** Провал любого `IStartable.Start()` (или
   регистрации каталога локаций) → `AggregateException` наружу ПОСЛЕ того,
   как все модули получили свою попытку (полная диагностика одной попытки);
   `_initialized` не выставляется — тик-луп заглушён, полуинициализированные
   модули не симулируют и не автосейвят. `GameBoot` глушит тик-луп и не
   входит в игру.
   **Retry = resume from failure (P1-10, Фазы 7/10):** повторный `Start()`
   ретраит ТОЛЬКО провалившиеся и ещё не стартованные модули — успешно
   завершившие `Start()` пропускаются (`_startedModules`): повторный Start
   модуля с EventBus-подписками = утечка токена + двойная обработка
   событий (runtime-подтверждено: SubscriberCount 1→2 за ретрай). Дублирующий
   слой — идемпотентные `Start()` всех модулей (гвард `_startCompleted`,
   флаг только при успехе — провал остаётся ретраимым; защищает и прямые
   вызовы вне бута, напр. повторную инстанциацию сцены адаптера).
   Страж — QA-сим AUDIT0922 секции G и J. Оставшийся бэклог: идемпотентность
   `Initialize()` отдельных сервисов при ЧАСТИЧНОМ провале модуля (P2-10/P2-11).
3. **Tick — FAIL-OPEN.** Исключение одного `ITickable` в рантайме
   логируется и пропускается — симуляция продолжается (изоляция сбоя).
   Зеркально fail-closed startup: единичная ошибка в установившемся
   рантайме не останавливает мир.
4. **Cycle detection — construction path.** Циклическая зависимость
   ловится на ПЕРВОМ повторном входе типа в текущую цепочку конструирования
   с точным путём «A → B → C → A» (раньше — только depth>50: 51 лишний
   конструктор до ошибки и нет пути). Depth-limit остаётся страховкой на
   случай циклов через фабрики. Кэш синглтонов не мешает: инстанс
   кэшируется ПОСЛЕ завершения конструирования — повторный вход в
   недостроенный тип по определению цикл. Страж — QA-сим AUDIT0922 секция H.
5. **World reset — FAIL-CLOSED (P1-9, Фазы 6/7/10).** Исключение
   `IWorldResettable.ResetWorld()` любого домена → `AggregateException`
   наружу из `WorldDomainResetPhase.ExecuteAsync()` (после того, как все
   домены получили попытку — полная диагностика, зеркально startup):
   оркестратор → `MarkAsFailed` + `SceneAssemblyFailedEvent` → rethrow →
   `GameSession.NewGame/LoadGame` возвращает сессию в `MainMenu`.
   Частично-сброшенный мир НЕ собирается дальше (фаза 0 — последующие
   Spawn/Init/Ready считали мир чистым). Rollback НЕ выполняется — reset
   не транзакционен (P2-20): контракт = «сброс упал → world-transition
   неуспешна», а не «мир частично жив». Единая семантика с LoadGame-путём
   (GameSession.ResetWorld-цикл — исключение выходит наружу).
   Страж — QA-сим AUDIT0922 секция I.
