...
### 1.8. Контракты жизненного цикла и ResolveAll (аудит 09.22, Фаза 4 — P2-12/P2-13/P2-14)

Контейнер и `GameEntryPoint` держат три явных контракта (раньше — молчаливые
инварианты, часть не выполнялась):
...
2. **Startup — FAIL-CLOSED.** Провал любого `IStartable.Start()` (или
   ...
   входит в игру. Ретрай `Start()` пересобирает списки с нуля. Бэклог:
   идемпотентность `Start()` отдельных сервисов (P2-10/P2-11).
