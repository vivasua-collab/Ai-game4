# Чит-меню разработки (CheatPanel, F2)

**Дата создания:** 2026-08-27 (Phase G). **Синхронизировано с кодом:** 2026-09-06 (аудит).
**Исходник:** `game/src/Adapter/UI/CheatPanel.cs` (~780 строк, #if DEBUG)
**Управление:** клавиша `F2` — открыть/закрыть панель в любой момент игры.

> **История:** изначально (2026-08-27) — F1, top-left плавающая панель.
> 2026-08-28 решением пользователя F1 закреплён за окном-справкой (Old School),
> чит-меню переехало на **F2** и стало **центрированным модальным окном** с затемнением и скроллом.
...
- **M2 (2026-09-03, секция физической боевки):** полное исцеление, спавн мишени-бандита, выдача лука в руки, дистанционная мишень + бандит за камнем (LOS-тест), стрелы ×50.

Все кнопки работают в DEBUG-сборке. В release-сборке класс `CheatPanel`
не компилируется благодаря директиве `#if DEBUG ... #endif`.

### Отключение в настройках (требование пользователя, выполнено)

- `GameSettings.CheatsEnabled` — персистентная настройка (user://settings.json).
- Чекбокс в настройках главного меню (MainMenuController).
- Гейт при нажатии F2: если выключено — окно не открывается, тост-уведомление.

---

## Структура UI

Центрированное модальное окно с затемнением фона (ModalOverlay) и внутренним скроллом.

```
⚡ ЧИТ-МЕНЮ (F2) — dev-only, #if DEBUG
─────────────────────────────────────────────────
▸ Уровень культивации:  [L1][L2]...[L9]
▸ Ци:                   [Заполнить Ци][+10000 Ци][Прорыв ▲]
...
▸ Экипировка:           [Оружие cycle][Броня cycle][Рандом]
                        [Оружие + зачарование]
▸ Легендарки:           [Легендарка оружие][Легендарка броня]
   (промо 20% /         [×20 легендарок: статистика оверкапа]
    оверкап 18%)
...
▸ M2-физическая боевка: [Полное исцеление][Мишень-бандит]
                        [🏹 Лук в руки][Дальний+мишень]
                        [Стрелы ×50][Мишень за камнем (LOS)]
─────────────────────────────────────────────────
[статус-строка]
```
...
### Легендарки (2026-08-26: Epic→Legendary промоушен + оверкап)

- **Легендарка оружие**: `EquipmentGenerator.GenerateLegendaryWeapon(level,
  subtype, seed)` — принудительный легендарный путь (Rarity=Legendary,
  гарант. зачарование, макс. бонусы, value ×3). Оверкап роллится по шансу
  18% (статы Damage/Durability по формулам L+1). Cycle по подтипам оружия.
- **Легендарка броня**: `GenerateLegendaryArmor(...)` — аналогично
  (Defense/Durability по формулам L+1 при оверкапе).
- **×20 легендарок: статистика оверкапа**: 20 генераций (оружие/броня
  поочерёдно) + `Verifier.Validate(...)` каждой. В статус: фактический %
  оверкапа и кол-во валидных; в лог — детали. Быстрая проверка шанса
  18% без ручного пересчёта (для вечернего теста на домашнем ПК).
- См. `LEVEL_BOUNDARIES.md` → раздел «Epic→Legendary промоушен и оверкап».
...
- ~~**Weapon switching system**: слоты 1/2 — только тосты, TBD~~ →
  **Реализовано (Phase 8 p2, 2026-09-03):** слоты 1/2 — полноценное
  переключение режимов ближнего/дальнего боя (SwitchToMeleeMode /
  SwitchToRangedMode, лук через WeaponMain с AttackRange > 2).
- **Сохранение TechniqueSlotService ISaveable**: SaveDataAggregator не
  собирает ISaveable автоматически — TBD (весь save/load отключён, Q8).
