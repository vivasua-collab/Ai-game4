...
> сейвы ломали загрузку после каждого изменения структур). F5/F9 не обрабатываются
> (без тоста), автосейва нет, пункт «Load Game» в меню всегда fallback в NewGame.
