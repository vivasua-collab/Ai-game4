...
| `GODOT_CHARGERQA_DEBUG=1` | ChargerSimDebug | R37-c (баг-репорт 23.09 «Зарядник не принимает камни Ци»): полный путь зарядника — A) инфраструктура (InputMap-действие charger_window/H, окно ChargerWindow создано, камни сидированы), B) честный отказ вставки без зарядника (тост-причина, предмет не потерян), C) генерация L3+ → надевание в Belt → IsChargerEquipped, гейт пояса (зарядник в Belt НЕ включает BeltSlotRow — раньше включал: баг «лекарство добавляется, камни молча отвергаются»), режим домена синхронизирован экипировкой (On), D) окно H открывается, контент виден, E) вставка камня через мост (слот-адресно R10: Guid кучки; предмет покинул инвентарь; MaxQi камня = канон 1024×см³, НЕ доменный дефолт BaseQi×mult), F) домен качает (камень истощается, буфер/ядро практика растут — метрика камня обязательна: TransferToPractitioner вне боя сливает буфер ядру, BufferQi≈0 не дефект), G) анти-дюп: частичный камень НЕ извлекается (тост-причина: стаки ItemId+Count без per-экземпляра), полный извлекается и возвращается, пустой «рассыпается» (не дюпается), H) сейв round-trip: MaxQi камня из моста не урезается reload'ом (баг RestoreState: доменный дефолт вместо сейв-поля), I) снятие зарядника → домен Off; настоящий пояс (armor_belt) → IsBeltEquipped=true (слоты 3-9 для лекарств — только с поясом). Скриншот: `GODOT_CHARGER_SHOT=<префикс>` (Xvfb + opengl3, без xauth — Xvfb вручную: `Xvfb :99` + `DISPLAY=:99` + `DOTNET_ROOT`). Сим №20 в qa_regression.sh |
| `GODOT_ANIMQA_DEBUG=1` | SpriteAnimSimDebug | G0 «Подготовка спрайтов» (директива 25.09): контур sprite-swap ДО доставки PNG — A) fallback: TryGetSheet(player_idle/walk)=null при отсутствии файлов (визуал идентичен процедурному), B) загрузчик: QA-плейсхолдеры _qa/ (полоса 6 кадров 384×64 + D2-сетка 5×6 384×320) через LoadSheetDirect — FrameCount/Rows/FrameSize/FrameAt-математика (прогресс→кадр, ряд row*N+col), тихий null на несуществующий путь, C) аниматор игрока: старт fallback (IsPng=false, Frame=0) → оверлей QA-листа → IsPng=true + прокрутка кадров; one-shot melee 0.42с (истекает), death-hold до NotifyRespawn, возврат fallback после сброса, D) оружие (I-11): ключи кэша sword|1|Common стабильны, размеры 32/48, PngHand/PngIcon=0 (PNG не доставлены), E) NPC (I-5): ResolveNpcAnim(moving/idle) → процедурный fallback, PNG-счётчик рендерера 0, F) звери (I-7): ResolveAnimalAnim → fallback (wolf-сэмпл; отсутствие зверей — WARN), G) реестр: 13 player + 6 npc + 5 animal. Регламент доставки — docs_v2/07_ui/SPRITE_DELIVERY.md; сим №21 в qa_regression.sh |
| `GODOT_AUDIT0922_DEBUG=1` | Audit0922SimDebug | Аудит 09.22 (внешний): инфраструктурные контракты — A) EventBus изоляция исключений + re-entrancy-дрен (P1-5: фан-аут не прерывается, стэл-очередь не протухает), B) санация имён слотов в Modules-layer SaveFileHandler (P1-8 path traversal), C) DI override prune: stale concrete-ключ + стражи мульти-форварда/last-wins/наследования ключа (P1-6), D) честный DeleteSave по слоям (P1-7), E) Trade/Corpse : IWorldResettable (P1-1/P1-4), F) ResolveAll — порядок = порядок регистрации, мёртвые регистрации не воскресают, ResolveAll<ISaveFileHandler> = ровно адаптер (P2-12, Фаза 4), G) startup fail-closed — AggregateException + заглушённый тик-луп + ретрай без «already initialised» (P2-13, Фаза 4), H) cycle detection по construction path с точным путём «A → B → A», без ложных срабатываний на цепочках (P2-14, Фаза 4), I) WorldDomainResetPhase fail-closed: исключение ResetWorld() домена = AggregateException наружу с именем виновника, все домены диагностируются за одну попытку, MarkAsCompleted недостижим (P1-9, Фазы 6/7/10), J) startup retry = resume from failure: успешно стартовавшие модули НЕ перезапускаются (SubscriberCount стабилен, StartCalls=1), провалившийся ретраится (P1-10, Фазы 7/10), K) DamageType.Pure не входит в Qi-буфер: Ци игрока/NPC не расходуется, поглощение 0; контроль Physical — Ци расходуется (P1-12, Фазы 9/10), M) Фаза 11 — time pipeline: честный catch-up TickCatchUpClock (инвариант учёта: моделирование+bulk+остаток = Σ(delta×speed); hitch 2с@Quick → 30/30 тиков), BulkAdvanceTicks календарь точен, ResetWorld сбрасывает Quick/Paused → Normal, фантомных Day/Month/Year после сброса мира нет (P1-13/P1-14/P1-15), N) Фаза 12 — item identity: ID уникальны при коллизионных сидах 5/1005 (counter-based), определения не подменяются, category-index без stale при смене категории, RestoreState заменяет каталог, ResetWorld чистит + ре-сеет канон (P1-16/P2-31/P2-32), O) Фаза 12 — equipment гейты: RequiredCultivationLevel=9 при уровне 1 → отказ (уровень 9 → успех), StatRequirements STR≥50 → отказ, одноручное → WeaponOff разрешено, двуручное → запрет (P1-17/P1-18), P) Фаза 12 — StorageRing persistence: ISaveable+IWorldResettable контракты, warm-load без утечки содержимого мира A, cold-load восстанавливает сейв, реактивация не чистит (P1-19), Q) Фаза 14 — stat progression: продюсеры §5.1/§5.2 (удар→STR, уклонение→AGI, урон→VIT, техника/медитация→INT), порог max(1,floor(stat/10)), ConsolidateSleep по §6.2/§6.4 (8ч → +0.2 + остаток; <4ч — нет; delta≥thr → +1), капы INT=15/отрицательные/вторичные отвергаются, sleep-pipeline StartSleep(8ч)→480 тиков→авто-вейк+закрепление (P1-20/P2-43/P2-49), R) Фаза 14 — Revive оживляет тело (vital Head/Heart: HealPart/Reattach), dash-цель клэмпится в границы мира (P2-44/P2-45), S) Фаза 11 P2-бэклог — автосейв по МИРОВЫМ тикам (мини-DI: фейк ITimeService/ISaveService/Playing-сессии + SaveModule; разностная схема 29→59=30 мин → сейв, ResetWorld обнуляет отсчёт — process-остаток 10029 не триггерит, первый автосейв нового мира ровно на 30-м тике, bulk-скачок не теряет каденцию), единая pause-authority (Session.Pause → TimeService.Paused на реальной сессии), Resume восстанавливает скорость до паузы (Fast), IsPaused ⇒ DeltaTime==0, Load канонизирует три представления (дата первична, TickCount=минуты с 06:00 дня 1, рассинхронный блок 777/42 → пересчёт), WorldTime/RestoreState отвергают month=13/day=0/hour=29/minute=-10/год до эпохи (ArgumentOutOfRangeException → битый сейв = отказ загрузки), WorldConfig без мёртвых Start*/AutoSaveIntervalTicks/BaseTickRate (рефлексия) (P2-26…30 + P3-11), T) Фаза 12 P2-бэклог items/trade — валютная saturating-арифметика (Add у верхнего края int → int.MaxValue, SetBalance(-5) → 0), тотал сделки в long (покупка 6e9 > int.MaxValue → честный отказ ДО списания, SpendCalls=0; продажа 2.85e9 → насыщение int.MaxValue — не молча 0 и не минус), Inventory RestoreState валидирует слоты (count>0, ≤MaxStack, Enum.IsDefined category/rarity, ItemId в БД, _itemCountCache не отравлен), переноска — ВЛАДЕЛЬЧЕСКИЙ КАНОН 50/100 (user request 2026-08-22; P2-36 отклонён by-user: прямые указания владельца выше аудитора и документации; T4a/T4b — страж значений) (P2-33/34/35 + P2-36 by-user). Прогон до фиксов = runtime-подтверждение дефектов (чекпоинты 09_22_r32–r34 + фазы 11–14: 32 DEFECT + R36-a: 12 S-DEFECT + R36-b: 5 T-DEFECT, логи 09_22_r35_prefix/postfix + 09_22_r36a_prefix/postfix + 09_22_r36b_prefix/postfix), после — регрессионный страж; сим №19 в qa_regression.sh |

### 0.2. Прогон регрессии — `tools/qa_regression.sh` (2026-09-15)

Раннер регрессии — `bash tools/qa_regression.sh [sim…]` из корня репо (без
аргументов — все симы реестра; аргумент — имя сима `COMBAT_SIM` или пара
